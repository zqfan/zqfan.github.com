...
注意：API v1在Havana版本开发中被标记为Deprecated，在Juno版本中已将其彻底移除。

注意：Ceilometer不支持查询API版本信息，但是其他服务例如Nova，Keystone等是支持的。详见Bug [https://bugs.launchpad.net/ceilometer/+bug/1350076](https://bugs.launchpad.net/ceilometer/+bug/1350076)。对该功能的支持已经在Kilo版本中实现。
...
| 参数名 | 参数类型 | 约束 | 必选 | 备注 |
|:-------|:---------|:-----|:-----|:-----|
| period | integer | >=0 | NO | 统计区间时间长度，单位秒
...
如果没有指定任何时间段，则统计是从有记录以来到最近的一次的全部统计。

period参数指定了统计区间的时间长度，默认为0，即只有一个统计区间。
如果指定了，例如86400，即一天，则会按照86400秒对时间进行分段，每段内单独进行统计。

如果没有指定起始时间，则第一个period的起始时间和第一个sample的时间戳一致。
如果指定了起始时间，则和指定的起始时间保持一致。

例如：

~~~
ceilometer -d statistics -m image -p 86400 -q start=2015-02-01T12:34:56
+--------+---------------------+---------------------+-----+-----+-----+-------+-------+----------+---------------------+---------------------+
| Period | Period Start        | Period End          | Max | Min | Avg | Sum   | Count | Duration | Duration Start      | Duration End        |
+--------+---------------------+---------------------+-----+-----+-----+-------+-------+----------+---------------------+---------------------+
| 86400  | 2015-02-01T12:34:56 | 2015-02-02T12:34:56 | 1.0 | 1.0 | 1.0 | 144.0 | 144   | 85800.0  | 2015-02-01T12:43:53 | 2015-02-02T12:33:53 |
| 86400  | 2015-02-02T12:34:56 | 2015-02-03T12:34:56 | 1.0 | 1.0 | 1.0 | 39.0  | 39    | 22801.0  | 2015-02-02T12:43:53 | 2015-02-02T19:03:54 |
+--------+---------------------+---------------------+-----+-----+-----+-------+-------+----------+---------------------+---------------------+
~~~

duration是指统计区间内，第一个出现的sample和最后一个出现的sample的时间的差。
duration的起始时间就是第一个sample出现的时间，duration的结束时间就是最后一个sample出现的时间。
...
注意：resource在有些数据库中是没有timestamp字段的（例如MySQL），API返回的first_sample_timestamp和last_sample_timestamp是临时计算的。
过滤字段里的start_timestamp和end_timestamp针对的是sample而言的，在数据库里查询时与sample的timestamp字段匹配。
